...
(define (pred p)
  (if (equal? p '()) (eopl:error "pred error: zero has no predecessor!") (car p)))
...
;; Type signature: (p- peano peano) -> peano
(define (p- a b)
  (cond [(and (equal? (pz? a) #t) (equal? (pz? b) #f)) (eopl:error "subtraction error: a < b!")]
        [(equal? (pz? b) #t) ]
...
(define (p-prime? p)
  (cond [(or (pz? p) (pz? (pred p))) #f]
        [else (p-prime-helper p (pred p))]))

(define (p-prime-helper p n)
  (cond [(pz? (pred n)) #t]
        [(pz? (p% p n)) #f]
        [else (p-prime-helper p (pred n))]))
