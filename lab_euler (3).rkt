...
(define (get-vertices G)
 (usort (cond [(equal? G '()) '()]
        [else (cons (car(car G)) (cons (car(cdr(car G))) (get-vertices (cdr G))))]))) 
...
(define (degree G v)
  (define (is-v? x y) (if (= x y) 1 0))
  (cond [(equal? G '()) 0]
        [else (+ (+ (is-v? (car(car G)) v) (is-v? (car(cdr(car G))) v)) (degree (cdr G) v))]))
...
(define (all-even? G)
  (define  (check-all-verts v)
    (cond [(equal? v '()) #t]
          [(not(even? (degree G (car v)))) #f]
          [else (check-all-verts (cdr v))]))
  (check-all-verts (get-vertices G)))
...
(define (adjacent G v)
  (usort(cond [(equal? G '()) '()]
        [(equal? (car(car G)) v) (cons (car(cdr(car G))) (adjacent (cdr G) v))]
        [(equal? (car(cdr(car G))) v) (cons (car(car G)) (adjacent (cdr G) v))]
...
(define (connected? G)
  (define (check g c)
    (cond [(equal? g '()) #t]
          [(or (equal? (member (car(car g)) c) #f) (equal? (member (car(cdr(car g))) c) #f)) #f]
          [else (check (cdr g) c)]))
  (define (run-through Graph g) 
    (cond [(equal? g '()) #t]
          [(not(or (check G  (dfs G '() (car(car g)))) (check G  (dfs G '() (car(cdr(car g))))))) #f]
          [else (run-through Graph (cdr g))]))
  (run-through G G))
