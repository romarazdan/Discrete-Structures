...
(define (CRT-exists? cong-sys)
  (cond [(equal? cong-sys '()) #t]
        ...
        [(= (euclid-gcd (car(cdr(car cong-sys))) (car(cdr(car(cdr cong-sys))))) 1) (and (CRT-exists? (cons (car cong-sys) (cdr(cdr cong-sys)))) (CRT-exists? (cdr cong-sys)))]
        [else #f]))
...
(define (CRT-helper cong-sys m)
  (if (CRT-exists? cong-sys)
       (cond [(equal? cong-sys '()) 0]
        [else (+ (* (*(car(car cong-sys))  (quotient m (car(cdr(car cong-sys))))) (mul-inv (quotient m (car(cdr(car cong-sys))))  (car(cdr(car cong-sys))))) (CRT-helper (cdr cong-sys) m))]) 0))
...
(define (CRT cong-sys)
  (cond [(not (CRT-exists? cong-sys)) -1]
        [else (modulo (CRT-helper cong-sys (m cong-sys)) (m cong-sys))]))