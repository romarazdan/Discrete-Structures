...
(define (encrypt plaintext key)
  (cond [(equal? plaintext "") '()]
        [else (cons (mod-exp (char->integer (car (string->list plaintext))) (car(cdr key)) (car key)) (encrypt (substring plaintext 1) key))]))  
...
(define (brute-force c m e d)
  (cond [(equal? d e) '()]
        [else (cons (decrypt c (list m d)) (brute-force c m e(- d 1)))]))
