...
(define (compose RO RI)
  (make-set (cond [(null? RI) '()]
        [else (append (compose-helper RO (car RI)) (compose RO (cdr RI)))])))


(define (compose-helper RO R)
  (cond [(null? RO) '()]
        [(equal? (car (cdr R)) (caar RO)) (cons (list (car R) (car (cdr (car RO)))) (compose-helper (cdr RO) R))]
        [else (compose-helper (cdr RO) R)]))
...
(define (transitive-closure R)
  (make-set (transitive-closure-helper R (cardinality R))))

(define (transitive-closure-helper R N)
  (cond [(= N 0) '()]
        [else (union (transitive-closure-helper R (- N 1)) (power R N))]))
...
(define (EQ-relation? R n)
  (if (and (and (transitive? R) (symmetric? R)) (reflexive? R n)) #t #f))
